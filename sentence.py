...
def vocab_make():
    ...
    if not os.path.isfile('translate_english.txt'):
    ...
    if not os.path.isfile('translate_korea.txt'):
        with open('translate_korea.txt', 'w', encoding='utf8') as f:
            f.write('\n'.join(df['원문']))

        spm.SentencePieceTrainer.Train(
        input='translate_korea.txt', model_prefix='translate_korea', vocab_size=30000, model_type='bpe', pad_id=0, unk_id=1, bos_id=2, eos_id=3)
